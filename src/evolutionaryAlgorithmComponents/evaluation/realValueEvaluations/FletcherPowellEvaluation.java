...
public class FletcherPowellEvaluation extends AbstractMathFunction implements HasProperties{
	...
	// Evaluations budget
	private final static int EVALS_LIMIT = 1000000;
	...
	private final static String regular = "false";
	private final static String separable = "false";
	private String evaluations = Integer.toString(EVALS_LIMIT);
	...
	private double[] a = {0.435934, 0.550595, -1.283410, -0.0734284, -2.6051900, 2.104100, 1.867540, -3.012750, 0.8628350,  0.0666833, 2.336110, -0.658149, -3.112430, -3.0755600, 0.8418540, -0.692549, 3.062840, -0.917399, 0.2111350, -1.4526100, 2.482440, 2.008340, 0.906190, -0.1087510, 0.6348730, 1.458810, 1.240920, 2.303110, -2.3116000, -2.1476100};
	...
	// The Fletcher-Powell function. The global minimum is located at a
	@Override
	public double f(double[] x) {

		if(x.length!=10) throw new IllegalArgumentException();
		...
		for(int i=0; i<x.length; i++){
			...
			for(int j=0; j<x.length; j++){
				Ai += A[i][j]*Math.sin(a[j]) + B[i][j]*Math.cos(a[j]);
				Bi += A[i][j]*Math.sin(x[j]) + B[i][j]*Math.cos(x[j]);
			}
